...
!!    Jesusedel:Aqui modifico la ecuacion de Friedman, agregando la funcion de 
!!    la materia oscura con la ecuacion 
!!    rho_c(a)=Omega_c/a^3[1+(a_c/a)^q_n]^(1/q_n) 
!  8*pi*G*rho*a**4.
!jesusedel

        
          if(a.LT.a_c)then
         grhoa2=grhok*a2+grhoc*a_c*exp(-r_vc*r_vc*(a_c*a_c-1)/2._dl) &
                +grhob*a+grhog+grhornomass
        else
	grhoa2=grhok*a2+grhoc*a*exp(-r_vc*r_vc*(a_c*a_c-a_c*a_c/a2)/2._dl) &
                +grhob*a+grhog+grhornomass
 ...
 !	jesusedel:Aqui el numero basico de ecuaciones es 5, 1 la ecuacion para a(t), 
!	2 el potencial eta,3 la perturbacion de densidad de DM,4 perturbacion de 
!	densidad de Bariones,5 perturbacion de velocidad para los bariones, 
! 	para agregar la ecuacion de la perturbacion de velocidad de DM.
        integer, parameter :: basic_num_eqns = 6
...
!	jesusedel:Aqui se neceista agregar la variable "vc" para 
! 	perturbacion de velocidad de DM y la variable vcdot por 
!	consistencia con la notacion del programa.
        real(dl) vc, vcdot, clxcex, grhocex_t,wc
    ...
    vb  =y(5)
    vbdot =yprime(5)
!       jesusedel:definir la perturbacion de velocidad de DM como la variable 
!	numero 6 del arreglo "y" y vcdot la misma pero del arreglo
!	"yprime".
   ...
   !=================================================================================
!        grhoc_t=grhoc/a
!	jesusedel:poner la funcion grho_c(a) en la definicion de grhoc_t    
        if(a.LT.a_c)then
         grhoc_t=grhoc*a_c*exp(-r_vc*r_vc*(a_c*a_c-1)/2._dl)/a2 
                
        else
	 grhoc_t=grhoc*exp(-r_vc*r_vc*(a_c*a_c-a_c*a_c/a2)/2._dl)/a 
         end if	
!==============================================================================
...
!=================================================================================
!  jesusedel      grho=grhob_t+grhoc_t+grhor_t+grhog_t+grhov_t
!        gpres=(grhog_t+grhor_t)/3+grhov_t*w_lam
!	f: Agregar el fluido extra.
	 if(a.LT.a_c)then
         wc= 1./3._dl
         else
	 wc= (1._dl/3._dl)*r_vc*r_vc*(a_c/a)**2
         end if	
!=================================================================================
...
!	jesusedel:
       ...
       dgq=grhob_t*vb+grhoc_t*vc*(1._dl+wc)
!=================================================================================        
...
!jesusedel: Agregar la presion del fluido DM.
        gpres=(grhog_t+grhor_t)/3+grhov_t*w_eff+wc*grhoc_t
!=================================================================================
    ...
    type(EvolutionVars) EV
    real(dl) y(EV%nvar)
!	jesusedel:definimos la variable Rpc15
	 real(dl) Rpc15, kc,kc2	
...
!	jesusedel:Hay que agregar el indice para la condicion inicial 
!	de la perturbacion de velocidad de DM.
        integer, parameter :: i_vc=13
   !	jesusedel:Aqui modificamos el numero maximo de indices poniendo el 
!	indice de la perturbacion de velocidad de DM.
!        integer, parameter :: i_max = i_vq
...
!	jesusedel:aqui necesitamos cambiar el parametro om, porque cuando 
!	se imponen las condiciones iniciales es cuando el universo es dominado
!	por radiacion, en epocas tempranas, en las cuales suponemos que DM se 
!	se comporta como radiacion, entonces cambiamos el Omega_m por Omega_b y
! 	Omega_R por Omega_R+Omega_c.
!
	om = (grhob)/sqrt(3*(grhog+grhonu+grhoc*a_c*exp(r_vc*r_vc*(1-a_c*a_c)/2._dl)))
        omtau=om*tau
...
!        Rb=1-Rc
!        Rp15=4*Rv+15
!	jesusedel:Cambiamo la definicion de las razones Ri ahora conciderando 
!	DM se comporta como radiacion en estas epocas tempranas. 

       Rv=grhonu/(grhonu+grhog+grhoc*a_c*exp(r_vc*r_vc*(1-a_c*a_c)/2._dl))
!       
        Rg =grhog/(grhonu+grhog+grhoc*a_c*exp(r_vc*r_vc*(1-a_c*a_c)/2._dl))
        Rc=grhoc*a_c*exp(r_vc*r_vc*(1-a_c*a_c)/2._dl)/(grhonu+grhog+grhoc*a_c*exp(r_vc*r_vc*(1-a_c*a_c)/2._dl))

        Rb=1
        Rp15=4*Rv+15
!	jesusedel:Definimos la cantidad Rpc15
        Rpc15=4*Rc+15
!===============================================================================
    ...
    if (CP%Scalar_initial_condition/= initial_adiabatic) then
      ...
      !=============================================================================
        !        initv(1,i_clxc)=initv(1,i_clxb)
!	jesusedel:Modificamos la condicion inicial para la perturbacion 
!       de    densidad de DM.
!        initv(1,i_clxc)=initv(1,i_clxb)
	
	if(a.LE.a_c) then 
	initv(1,i_clxc)=initv(1,i_clxg)
	initv(1,i_vc)=-chi*4._dl/3._dl*EV%Kf(1)*(4*Rc+23)/Rpc15*x3/27
	else
	initv(1,i_clxc)=0.75_dl*initv(1,i_clxg)
        initv(1,i_vc)=0
	end if
!=============================================================================
    ...
    y(3)=InitVec(i_clxc)
!================================================================================
 !	jesusedel:Aqui agregamos la linea para definir la condicion inicial 
!	para el arreglo y.
	y(6)=InitVec(i_vc) 
!================================================================================  
    ...
    real Arr(:)
!	jesusedel;Agregar la delta de CDMex
	real(dl) clxcex,a2
    real(dl) y(EV%nvar)

    a    = y(1)
!jesusedel: Aqui defino el parametro 
	a2   = a**2
    clxc = y(3)
    clxb = y(4)
    if (EV%no_nu_multpoles) then
   ...
   !	jesusedel:Ponemos la funcion grho_c(a) en lugar de de grhoc.
!
!
!         dgrho = dgrho+clxc*(grhoc/a)*(1+(a_c/a)**q_n)**(1/q_n) + (clxb*grhob)/a 
!         grho =  grho+(grhoc/a)*(1+(a_c/a)**q_n)**(1/q_n)+(grhob)/a
	
        if(a.LT.a_c)then
   dgrho = dgrho+clxc*(grhoc/a2)*a_c*exp(r_vc*r_vc*(1-a_c*a_c)/2._dl)+& 
           clxb*grhob/a

   grho =  grho+(grhoc/a2)*a_c*exp(r_vc*r_vc*(1-a_c*a_c)/2._dl)+ &
      grhob/a
         else
dgrho = dgrho+clxc*(grhoc/a)*exp(-r_vc*r_vc*(a_c*a_c-a_c*a_c/a2)/2._dl)+& 
           clxb*grhob/a

   grho =  grho+(grhoc/a)*exp(-r_vc*r_vc*(a_c*a_c-a_c*a_c/a2)/2._dl)+ &
      grhob/a
	end if  
    ...
    real(dl) a,a2,dz,z,clxc,clxb,vb,clxg,qg,pig,clxr,qr,pir
!	jesusedel: Agregamos las nuevas variables vc y vcdot
	real(dl) vcdot,vc,wc,dwc,ca2c,qnn,kc,cs2c,grhocex_t,clxcexdot,clxcex
    real(dl) E2, dopacity
    ...
    !  CDM variables
    clxc=ay(3)
!	jesusedel:Se agrega vc, la cual es la parte 6 del arreglo ay.
	vc=ay(6)
   ...
   !        grhoc_t=grhoc/a
!=================================================================================
!	jesusedel:Se cambia grhoc por la funcion grho_c(a).
	if(a.LT.a_c)then	
        grhoc_t=(grhoc/a2)*a_c*exp(r_vc*r_vc*(1-a_c*a_c)/2._dl)
	else 
	grhoc_t=(grhoc/a)*exp(-r_vc*r_vc*(a_c*a_c-a_c*a_c/a2)/2._dl)
	end if
!=================================================================================
...
! 
    ...
    dgrho=grhob_t*clxb+grhoc_t*clxc
   !jesusedel: Agregamos al momento al momento total el momento de DM.
!  8*pi*a*a*SUM[(rho_i+p_i)*v_i]
 !       dgq=grhob_t*vb
dgq=grhob_t*vb+grhoc_t*vc*(1._dl+wc)
    ...
    !
   !================================================================================
!	jesusedel:Aqui ponemos la ecuacion modificada de DM.
!  CDM equation of motion
 !        clxcdot=-k*z
	if(a.LT.a_c)then
       dwc=0
	else 
	dwc=(-2._dl/3._dl)*(r_vc*r_vc/a)*(a_c/a)**2
	end if
        ca2c = wc-a*dwc/(3._dl+3._dl*wc)  
	

	cs2c=ca2c


	clxcdot=-3._dl*adotoa*(cs2c-wc)*(clxc+3._dl*adotoa*(1._dl+wc)*vc/k) &
       -(1._dl+wc)*k*vc-3._dl*adotoa*adotoa*a*dwc*vc/k-(1._dl+wc)*k*z


        ayprime(3)=clxcdot

         vcdot=-adotoa*(1._dl-3.*cs2c)*vc + k*cs2c*clxc/(1._dl+wc)
...
!================================================================================
    ...
    if (EV%TightCoupling) then
        !  ddota/a
!===========================================================================
   !	jesusedel:Agregamos a la presion total, la contribucion desde DM.
!            gpres=gpres+ (grhog_t+grhor_t)/3 +grhov_t*w_lam


	
	 gpres=gpres+(grhog_t+grhor_t)/3 +grhov_t*w_eff+grhoc_t*wc
