    ...
    character(LEN=*), parameter :: version = 'Dec13'
    ...
    integer, parameter :: max_transfer_redshifts = 400
...
!===============================================================================
!	 jesusedel:Aqui agragamos el parametro q_n y a_c para nuestro modelo
         character(LEN=1024) evolucionfilec, sigmafilec
         real(dl)  :: q_nn, fixqpc, omegacex
	 real(dl)  :: N_cc, r_vcc, c2hc,c2cc, indpruebac, Npcc
...
!=============================================================================
	...
	!===============================================================================
!	jesusedel:definimos los parametros a_c y q_n
	character(LEN=1024) evolucionfile,sigmafile
        real(dl) a_c,q_n,r_vc,c2h,c2c,grhocex,fixqp
	integer  :: indprueba,Npc
!===============================================================================
    ...
    !H0 is in km/s/Mpc
	!===============================================================================
!	jesusedel:Definimos los parametros q_n y a_c
	evolucionfile=CP%evolucionfilec
        sigmafile=CP%sigmafilec
	fixqp=CP%fixqpc
!	q_n=CP%q_nn
	a_c= 0.00001*exp(-0.5*CP%N_cc)
	r_vc=exp(-1.*CP%r_vcc)
!	c2h=CP%c2hc
!	c2c=CP%c2cc
!	indprueba=CP%indpruebac 
	Npc=CP%Npcc
!===============================================================================
...
!===============================================================================
! 	jesusedel:Aqui tenemos que modificar la razon adotrad=H_0 Omega_R^1/2
!	agregando DM.Y tambien modificamos el valor de a_c en termino de a_eq
	

  !     a_c=a_c*(grhog+grhornomass+sum(grhormass(1:CP%Nu_mass_eigenstates))) &
!	/(grhob+grhoc)
			
           adotrad = sqrt((grhoc*a_c*exp(r_vc*r_vc*(1-a_c*a_c)/2.)+grhog+grhornomass+ &
			sum(grhormass(1:CP%Nu_mass_eigenstates)))/3)      
!======================================================================= 
...
500     if (L< lmax_extrap_highl) &
   ...
   !==================================================
 !                              Transfer_tot=7
!       	jesusedel:Agregar el indice para CDMex
	Transfer_cdmex= 7,Transfer_tot=8
!=================================================================================
    ...
    !JD 08/13 Changes in here to PK arrays and variables
    integer j_PK
    call CreateTxtFile(sigmafile,15)
    do in=1, CP%InitPower%nn
        ...
        do j_PK=1, CP%Transfer%PK_num_redshifts
            ...
            write (15,'(2E15.5)') real(CP%Transfer%redshifts(j)), real(MTrans%sigma_8(j_PK,in)) 		
        end do
    end do
	close(15)
...
!	jesusedel: Cmbiar el valor de esto	
 a_verydom = AccuracyBoost*5*(grhog+grhornomass+grhoc*a_c*exp(r_vc*r_vc*(1-a_c*a_c)/2.))/ &
                     (grhob)
!=================================================================================
...
!jesusedel: modifico el tiempo de zeq

if(a_c.LT.&
(grhog+grhornomass+sum(grhormass(1:CP%Nu_mass_eigenstates)))/(grhob+grhoc) )&
then
...
ThermoDerivedParams( derived_zEQ ) = 1/a_c -1
...
!=================================================================================
  !	jesusedel:agrego la deltac y tauc
	real (dl) tauc,deltac
!=================================================================================
...
!=========================================================================
!jesusedel: Defino los parametros deltac y tauc.
	if(CP%WantTransfer) then

	tauc=tauminn
	deltac=(taurst-tauc/100)/10000
!	jesusedel: calcula los pasos antes de tauc
	 call Ranges_Add_delta(TimeSteps, tauc, taurst, deltac)       
      	end if
!=========================================================================
