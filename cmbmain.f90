...
!========================================
    ...
    external dtauda
!	jesusedel: aqui pongo la definicion de fixq.
	h=CP%h0/100
	fixq=fixqp
!==========================================
...
!jesusedel
 		call CreateTxtFile(evolucionfile,5)
...
!    end if
        ...
        j=1
        do while(j.LE.Npc .and. y(1).LE.a_c )
	tauend = taustart+(j-1)*(CP%tau0-taustart)/Npc
	...
	write (5,'(8E15.5)') 1/y(1)-1,y(1),delta, growth,y(3), y(6), y(4), y(EV%g_ix)
         j=j+1
        end do
	...
	y(6)=y(6)*r_vc
	do while(jj.LE.Npc)
	tauend = taustart +(jj-1)*(CP%tau0-taustart)/Npc
	 call GaugeInterface_EvolveScal(EV,tau,y,tauend,tol1,ind,c,w)
             yprime = 0
            call derivs(EV,EV%ScalEqsToPropagate,tau,y,yprime)
            adotoa = 1/(y(1)*dtauda(y(1)))
    ...
    !jesusedel:Cambio la forma de evolucionar las perturbaciones para incluir el cambio
!en la velocidad del fluido de DM.
!     Begin timestep loop.
...
!     Begin timestep loop.
           ...
           j=2
	  do while(j.LE.TimeSteps%npoints.and.y(1).LT.a_c)
             tauend=TimeSteps%points(j)
...
!     output transfer functions for this k-value.
   ...
   y(6)=y(6)*r_vc
	...
	do while(j.LE.TimeSteps%npoints)
...
!     Calculation of transfer functions.
1011          if (CP%WantTransfer.and.itf <= CP%Transfer%num_redshifts) then
                if (j < TimeSteps%npoints) then
                  if (tauend < tautf(itf) .and.TimeSteps%points(j+1)  > tautf(itf)) then
...
!     output transfer functions for this k-value.
                  ...
                  if (abs(tau-tautf(itf)) < 1.e-5_dl) then
                           ...
                           if (j < TimeSteps%npoints) then
                            if (itf <= CP%Transfer%num_redshifts.and. &
                                TimeSteps%points(j+1) > tautf(itf)) goto 1011
                           end if
                  endif
...
!       real(dl) tau
 ...
 itf=1
           tol1=tol/exp(AccuracyBoost-1)
	!jesusedel:redefino atol
           if ( CP%Transfer%high_precision) tol1=tol1/10000
...
!     Calculation of transfer functions.
10011          if (CP%WantTransfer.and.itf <= CP%Transfer%num_redshifts) then
                if (j < TimeSteps%npoints) then
                  if (tauend < tautf(itf) .and.TimeSteps%points(j+1)  > tautf(itf)) then
 ...
 !    output transfer functions for this k-value.
                  ...
                  if (abs(tau-tautf(itf)) < 1.e-5_dl) then
                          ...
                          if (j < TimeSteps%npoints) then
                            if (itf <= CP%Transfer%num_redshifts.and. &
                                TimeSteps%points(j+1) > tautf(itf)) goto 10011
                           end if
                  endif
